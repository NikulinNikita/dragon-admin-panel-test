Admin.Modules.register('form.elements.dependent-select', () => {
    $('.js-data-smartDepSelect').depdrop2()
}, 0, ['bootstrap::tab::shown']);

function changeDepObj(self, $select) {
    let $depVal = $select.val();

    $.each(self.$element, function (i, elem) {
        let $depKey = $(elem).attr('depkey') ? $(elem).attr('depkey') : $select.attr('name');
        let $depTarget = $(elem).attr('deptarget') ? $(elem).attr('deptarget') : 'queryfilters';
        let $resultedValue;

        if($depTarget && $depTarget === 'model') {
            let $uppercasedModelVal = $depVal.charAt(0).toUpperCase() + $depVal.slice(1);
            $resultedValue = $depVal.indexOf('App\\Models\\') === -1 ?  'App\\Models\\' + $uppercasedModelVal : $uppercasedModelVal;
        } else {
            let $queryFilters = $(elem).attr('queryfilters') ? JSON.parse($(elem).attr('queryfilters')) : null;
            $.each($queryFilters, function (key, group) {
                if(group[0] === $depKey) {
                    group[group.length === 3 ? 2 : 1] = $depVal;
                }
            });
            $resultedValue = JSON.stringify($queryFilters);
        }
        $(elem).attr($depTarget, $resultedValue)
    });
}
...
(function (factory) {
    ...
    DepDrop.prototype = {
        ...
        listen: function (i, depends, len) {
            var self = this;
            $('#' + depends[i]).on('depdrop.change select2:select krajeeselect2:cleared', function (e) {
                var $select = $(this);

                changeDepObj(self, $select);
    ...
    $.fn.depdrop2 = function (option) {
        ...
        this.each(function () {
            ...
            if (!data) {
                ...
                $.extend(true, opts, $.fn.depdrop2.defaults, $.fn.depdropLocales.en, loc, options, self.data());
