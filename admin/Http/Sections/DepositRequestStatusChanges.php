...
class DepositRequestStatusChanges extends BaseSection
{
    ...
    public function onDisplay($scopes = [])
    {
        ...
        $display->setApply(function ($query) use ($scopes, $model) {
            if (array_get($scopes, 'deposit_request_id')) {
                $query->where("{$model->getTable()}.deposit_request_id", array_get($scopes, 'deposit_request_id'));
            }
        });
        $display->setParameters(['deposit_request_id' => array_get($scopes, 'deposit_request_id')]);
        ...
        if ( ! $scopes) {
            $display->setColumns([
                AdminColumn::sText('deposit_request_id', $model),
            ]);
        } else {
